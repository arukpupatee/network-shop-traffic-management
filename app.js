...
zabbix.login(config.zabbix.username, config.zabbix.password, function (res) {
  ...
  if (!req.isAuthenticated()) {
    ...
    get_router_groups(function (router_groups) {
      async.each(router_groups, function (group, callback_group) {
        obj.btn_group_list.push({
          'title': group.area,
          'src': '/dashboard?show=' + group.area,
          'active': false
        });
        callback_group(null);
      }, function (err) {
              ...
              if (router_groups_id_data[req.query.show] === undefined) {
                ...
                async.each(router_groups, function (group, callback_group) {
                  ...
                  get_alert_hosts_and_hosts_traffic(area.group_id, function (results) {
                    ...
                    let traffic_counter = 0;
                    async.each(area.hosts_with_traffic, function (host, callback_avg) { //find avg
                      if (area.hosts_with_traffic.length != 0) {
                        if(host.traffic_avg_peak != undefined){
                          total_traffic += host.traffic_avg_peak;
                          total_traffic_percent += Number(host.traffic_avg_peak_percent);
                          traffic_counter += 1;
                        }
                      }
                      callback_avg(null);
                      ...
                      if (area.hosts_with_traffic.length != 0) {
                        let avg = total_traffic / traffic_counter;
                        area.avg_traffic = math.unit(avg, 'm').format({
                          notation: 'fixed',
                          precision: 2
                        }).replace('m', 'b') + 'ps';
                        area.avg_traffic_percent = (total_traffic_percent / traffic_counter).toFixed(2);
                      } else {
                        area.avg_traffic = "-";
                        area.avg_traffic_percent = "-";
                        area.traffic_avg_peak_top = "-";
                        area.traffic_avg_peak_percent_top = "-";
                      }
                      obj.area.push(area);
                      callback_group(null);
                    });
                ...
                get_alert_hosts_and_hosts_traffic(obj.group_id, function (results) {
                  ...
                  if (obj.hosts_with_traffic.length != 0) {
                    ...
                    async.each(obj.hosts_with_traffic, function (host, callback_avg) { //find avg
                      ...
                      let avg = total_traffic / traffic_counter;
                      obj.avg_traffic = math.unit(avg, 'm').format({
                        notation: 'fixed',
                        precision: 2
                      }).replace('m','b') + 'ps';
                      obj.avg_traffic_percent = (total_traffic_percent / traffic_counter).toFixed(2);
                    ...
                    obj.traffic_avg_peak_top = "-";
                    obj.traffic_avg_peak_percent_top = "-";
                    res.render('pages/dashboard-part', obj);
                  }
                });
              }
    ...
    get_hosts_traffic(group_id, function (hosts_with_traffic) {
      ...
      if (hosts_with_traffic.length != 0) {
        ...
        async.each(hosts_with_traffic, function (host, callback_each) {
          ...
          let avg = total_traffic / traffic_counter;
          res_obj.avg_traffic = math.unit(avg, 'm').format({
            notation: 'fixed',
            precision: 2
          }).replace('m', 'b') + 'ps';
          res_obj.avg_traffic_percent = (total_traffic_percent / traffic_counter).toFixed(2);
          res.json(res_obj);
        });
      } else {
        ...
        res_obj.traffic_avg_peak_top = "-";
        res_obj.traffic_avg_peak_percent_top = "-";
        res.json(res_obj);
      }
    });
  }
      ...
      async.each(r_maps, function (map, callback_router_maps) {
        ...
        get_detail_maps(function (d_maps) {
          async.each(d_maps, function (map, callback_detail_maps) {
            detail_maps.push({
              'title': map.area.replace(config.maps.detail_map_name_ends_with, ""),
              'key': map.area,
              'href': '/map?show=' + map.area,
              'active': false
            });
      ...
      async.each(host.triggers, function (trigger, callback_trigger) {
        if (trigger.value == "1") {
          if (is_json_in_array_match(trigger.description, config.alerts.triggers, "name")) {
            json_in_array_to_json(config.alerts.triggers, "name", function (t) {
              triggers = t;
            });
  ...
  zabbix.get_hosts_item_by_groupids(groupids, function (hosts) {
    ...
    async.each(hosts, function (host, callback_host) {
      ...
      async.each(host.items, function (item, callback_item) {
        ...
        } else if (item.key_.startsWith("ifIAvgOctets[") && is_string_include_substring_in_array(item.key_, config.traffics.interfaces)) {
          host_with_traffic.traffic_avg_peak = Number(item.lastvalue);
        } else if (item.key_.startsWith("ifIAvgOctetsPercent[") && is_string_include_substring_in_array(item.key_, config.traffics.interfaces)) {
          host_with_traffic.traffic_avg_peak_percent = Number(item.lastvalue).toFixed(2);
        } 
        callback_item(null);
      }, function (err) {
        if (err) throw err;
        if ((host_with_traffic.icmpping == 1) && (typeof host_with_traffic.incoming_traffic != "undefined") && (host_with_traffic.incoming_traffic != 0)) {
          host_with_traffic.traffic = math.unit(host_with_traffic.incoming_traffic, 'm').format({
            notation: 'fixed',
            precision: 2
          }).replace('m', 'b') + 'ps';
